    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '🏷️ Thể loại', value: genres, inline: true },
          { name: '🛠️ Nhà phát triển', value: devs, inline: true },
          { name: '📅 Ngày phát hành', value: release, inline: true },
          { name: '📊 Đánh giá', value: meta, inline: true },
          ...(trailerText ? [{ name: '🎥 Trailer', value: trailerText, inline: true }] : [])
        )
        .setFooter({ text: 'Dữ liệu từ Steam Store', iconURL: 'https://store.steampowered.com/favicon.ico' })
        .setTimestamp();
      ...
      console.error(err);
      await interaction.editReply('⚠️ Lỗi khi lấy thông tin game. Thử lại sau nhé!');
    }
  }
});
